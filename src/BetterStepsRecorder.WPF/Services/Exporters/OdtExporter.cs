...
namespace BetterStepsRecorder.WPF.Services.Exporters
{
    /// <summary>
    /// Exporter for OpenDocument Text (ODT) files
    /// </summary>
    internal class OdtExporter : ExporterBase
    {
        private List<ScreenshotInfo> _screenshots;
        public OdtExporter(List<ScreenshotInfo> screenshots)
            : base()
        {
        ...
        private void CreateManifestFile(string tempDir)
        {
            ...
            using (XmlWriter writer = XmlWriter.Create(manifestPath, settings))
            {
                ...
                {
                    if (!string.IsNullOrEmpty(screenshot.ScreenshotBase64))
                    {
                        string imageFileName = $"Pictures/step_{screenshot.ElementName}_{screenshot.ID.ToString().Substring(0, 8)}.png";
        ...
        private void CreateContentFile(string tempDir)
        {
            ...
            using (XmlWriter writer = XmlWriter.Create(contentPath, settings))
            {
                ...
                // Add each step
                foreach (var recordEvent in _screenshots)
                {
                    // Step header
                    writer.WriteStartElement("p", "urn:oasis:names:tc:opendocument:xmlns:text:1.0");
                    writer.WriteAttributeString("style-name", "urn:oasis:names:tc:opendocument:xmlns:text:1.0", "StepHeader");
                    writer.WriteString($"Step {recordEvent.ElementName}: {recordEvent._StepText}");
                    writer.WriteEndElement(); // text:p
                    ...
                    */
                    ...
                    if (!string.IsNullOrEmpty(recordEvent.ScreenshotBase64))
                    {
                        string imageFileName = $"Pictures/step_{recordEvent.ElementName}_{recordEvent.ID.ToString().Substring(0, 8)}.png";
                        
                        // Get image dimensions for proper aspect ratio
                        Size imageSize = GetImageDimensions(recordEvent.ScreenshotBase64);
                        ...
                        writer.WriteAttributeString("name", "urn:oasis:names:tc:opendocument:xmlns:drawing:1.0", $"Image{recordEvent.ElementName}");
        ...
        private void SaveImages(string tempDir)
        {
            ...
            foreach (var recordEvent in _screenshots)
            {
                if (!string.IsNullOrEmpty(recordEvent.ScreenshotBase64))
                {
                    string imageFileName = $"step_{recordEvent.ElementName}_{recordEvent.ID.ToString().Substring(0, 8)}.png";
                    string imageFilePath = Path.Combine(imagesFolder, imageFileName);
                    
                    try
                    {
                        // Convert base64 to image and save
                        byte[] imageBytes = Convert.FromBase64String(recordEvent.ScreenshotBase64);
